    ...
    methods: {
      async saveContact() {
        ...
        if (!this.errors.length) {
          ...
          if (this.contact.images.length) {
            formData.append('image', this.contact.images[0]);
          }

          let url = '/api/save_contact';
...
<template>
  <div class="">
    ...
    <form @submit.prevent="saveContact" class="max-w-sm mx-auto border border-blue-400 p-5 mt-5 rounded">
      <div class="border-b border-blue-300 mb-8">
        <h1 class="text-center mb-4 text-2xl">ADD CONTACT</h1>
      </div>
      <div class="mb-5">
      ...
      <div class="mt-5" v-if="imagePreviews.length">
        <h2 class="text-center text-2xl mb-4">Image Preview</h2>
        <div class="flex justify-center">
          <img :src="imagePreviews[0]" alt="Image Preview" class="h-40 w-40 object-cover"/>
          <button @click="removeImage(0)" class="ml-4 text-red-600 hover:text-red-800">Remove</button>
        </div>
      </div>
